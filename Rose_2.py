...
def prikupi_podatke_od_korisnika():
    print("=== Generator ruže vetrova u vektorskom formatu ===")
    
    ime_kraja = input("Unesite ime kraja (za naslov grafika i ime fajla, npr. Kaluđerica): ")
    
    try:
        lat = float(input("Unesite geografsku širinu (Latitude, npr. 44.75): "))
        lon = float(input("Unesite geografsku dužinu (Longitude, npr. 20.55): "))
    except ValueError:
    ...
    return ime_kraja, lat, lon, visina_param, start_date, end_date

# --- 1. KORISNIČKI UNOS ---
IME_KRAJA, LAT, LON, VISINA_PARAM, START_DATE, END_DATE = prikupi_podatke_od_korisnika()

# --- 2. PREUZIMANJE PODATAKA ---
print("\nPreuzimanje istorijskih podataka sa Open-Meteo API-ja...")
...
if 'hourly' not in podaci:
    print("Greška: API nije vratio očekivane podatke o satnim merenjima.")
    sys.exit(1)
...
print("Generisanje vektorskih fajlova...")
...
# Naslov sada koristi ime kraja koje korisnik unese
naslov_grafika = f"Grafikon 1. Ruža vetrova za period 2010 – 2025. godina, {IME_KRAJA}"
plt.title(naslov_grafika, y=1.08, fontweight="bold")
ax.legend(loc='lower right', title="Brzina vetra (m/s)", bbox_to_anchor=(1.2, 0))

# --- 5. ČUVANJE U VEKTORSKE FORMATE ---
# Kreiramo sigurno ime fajla (bez razmaka i specijalnih karaktera ako ih ima)
sigurno_ime = "".join([c for c in IME_KRAJA if c.isalnum() or c in (' ', '_', '-')]).replace(" ", "_")

fajl_svg = f"Ruza_vetrova_{sigurno_ime}.svg"
fajl_pdf = f"Ruza_vetrova_{sigurno_ime}.pdf"

# Čuvanje fajlova
plt.savefig(fajl_svg, format='svg', bbox_inches='tight')
plt.savefig(fajl_pdf, format='pdf', bbox_inches='tight')

print(f"\nUspesno završeno! Vektorski fajlovi su sačuvani kao:")
print(f" - {fajl_svg}")
print(f" - {fajl_pdf}")

# Opciono prikazivanje grafika na ekranu
plt.show()